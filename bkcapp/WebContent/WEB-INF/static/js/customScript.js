...
$('#accountUpdate').click(function(){
    var name = $("#name").val(),
        age = $("#age").val(),
        emailDetails = $("#emailDetailsInput").val();
        mobile = $("#mobile").val(),
        ...
        tbClients[sequenceNo] = JSON.stringify({    Name    : name,
                                                    Age  : age,
                                                    Email : emailDetails,
                                                    Mobile : mobile,
                                                    Password : password,
...
$('#propertySave').click(function(){
    var propflatno = $('#propflatno').val(),
        propsize = $('#propsize').val(),
        propowner = $('#propowner').val(),
        propaddress = $('#propaddress').val(),
        propcity = $('#propcity').val(),
        propstate = $('#propstate').val(),
        propCompleteAddress = propaddress + ", " + propcity + ", " + propstate;
...
var loadListData = function(){
    if($('#products').length > 0){
        //alert("1");
        $.ajax({
            url: 'http://localhost:8082/bkcapp/property/getHouses/0/', //change this with List property URL 
            type: 'GET',
            dataType: 'json',
            success: function(data){
              console.log(data);   
              ...
              if(data && data.length > 0){
                for (var i = 0; i <= data.length; i++) {
                  //  if(data[i].rentedState !== "2"){
                		console.log(data[i])
                        $('#products').append(" <div class='item  col-xs-4 col-lg-4'><div class='thumbnail'><img class='group list-group-image' src='http://placehold.it/400x250/000/fff' alt=' /><div class='caption'><h4 class='group inner list-group-item-heading'>"
                        						+data[i].houseSize+" Sq.Ft</h4><p class='group inner list-group-item-text'>"
                        						+data[i].aboutProperty+"</p><div class='row'><div class='col-xs-12 col-md-6'><p class='lead'>Rs."
                        						+data[i].rent+"</p></div><div class='col-xs-12 col-md-6'><a class='btn btn-success testClass' href='detailPage.html?id="
                        						+data[i].propertyRegNo+"' id='"
                        						+data[i].propertyRegNo+"'>View Detail</a></div></div></div></div></div>");
                        flagDom = true;
                   // }
                }
            }          
...
if($('#productDetail').length > 0){
    var searchParams = new URLSearchParams(window.location.search),
        ...
        console.log(keyval);    
    $.ajax({
        url: 'http://localhost:8082/bkcapp/property/gethouseDetails/'+keyval+'/', //change this with Property Detail URL 
        type: 'GET',
        dataType: 'json',
        success: function(data){
            console.log(data);   
            if(data.hasOwnProperty("username") && data.propertyRegNo === keyval){
